...
const { makeDotPng } = require(path.join(__dirname, '..', 'lib', 'tray_dot_png.js'));
...
const trayStatusDotIcons = new Map();

function getTrayStatusDotIcon(status) {
  if (trayStatusDotIcons.has(status)) return trayStatusDotIcons.get(status);
  const color = getTaskTrayStatusColor(status);
  if (!color) {
    trayStatusDotIcons.set(status, undefined);
    return undefined;
  }
  // nativeImage cannot decode SVG, so rasterize the dot to a PNG ourselves.
  const png = makeDotPng(color, 32);
  if (!png) {
    trayStatusDotIcons.set(status, undefined);
    return undefined;
  }
  ...
  // Downscale the supersampled source to menu-icon size for a crisp dot.
  const resized = image.resize({ width: 14, height: 14, quality: 'best' });
  resized.setTemplateImage(false);
  trayStatusDotIcons.set(status, resized);
  return resized;
}
...
function createTray(port) {
  ...
  const showTrayMenu = async () => {
    ...
    const projectItems = sorted.flatMap((p) =>
      getProjectTrayItems(p).map(({ label, status, task }) => ({
        label,
        status,
        icon: getTrayStatusDotIcon(status),
        click: async () => {
          try {
