 ...
 *
 * Local only: ~/.cursor/chats is not synced over ssh, so remote (source:'ssh') watches do not use
 * this reader (they keep the transcript-only question path).
 */
...
// Read the REMOTE conversation head over ssh and report whether it is a pending AskQuestion asked
// after `sinceMs`. cursor-agent runs headless on the remote (cursor-cli --source ssh), so its store.db
// lives on the remote — we run python on the host to copy store.db(+wal+shm) to a remote temp, read it
// read-only there (the same WAL-safe snapshot the local reader does), and return the head blob's hex.
// The hex is parsed HERE with the same helpers the local path uses, so the pending decision is
// identical local vs remote. `runSsh(host, cmd, timeoutMs)` is the transport (createSshRunner). Returns
// false on any failure (missing dir, torn read, ssh error) so a cold poll never false-positives.
async function remotePendingAskQuestion({ host, conversationId, sinceMs = 0, runSsh, timeoutMs = 4000 } = {}) {
  ...
  const params = Buffer.from(JSON.stringify({ conv: conversationId }), 'utf8').toString('base64');
  const py = [
    'import json,base64,os,sqlite3,tempfile,shutil,binascii',
    `P=json.loads(base64.b64decode("${params}").decode())`,
    'conv=P["conv"]; root=os.path.expanduser("~/.cursor/chats"); d=""',
    'try:',
    ...
    '        if br and br[0]: out["head_hex"]=br[0]',
    '    finally: conn.close()',
  ...
  try {
    ...
    res = JSON.parse(String(out || '').trim() || '{}');
  } catch {
    return false;
  }
  if (!res || !res.head_hex) return false;
  const startedAtMs = pendingAskQuestionStartedAtMs(extractJsonObjectsFromBuffer(Buffer.from(String(res.head_hex), 'hex')));
