...
function readHeadPendingGates(storeDbPath, config, opts = {}) {
  ...
  try {
    ...
    // meta.value is hex-encoded JSON TEXT carrying latestRootBlobId → the head blob id.
    const metaRes = spawnSync(sqlite3Path, [dest, 'SELECT hex(value) FROM meta LIMIT 1;'],
