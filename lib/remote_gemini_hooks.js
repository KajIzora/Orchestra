...
async function ensureRemoteGeminiHooks(remote, options = {}) {
  ...
  const py = [
    ...
    // Rotate an oversized hook-debug.log at install time (start of a round) — every hook appends a
    // full env dump, and a 36MB backlog behind the 256KB/tick tail reader was a false-clear flood
    // source in the 2026-07 ssh round. Install-time (not in-hook) so rotation never happens mid-run
    // under an active reader offset. One .1 backup is kept (os.replace overwrites the previous one).
    'dl=os.path.join(h,".gemini","antigravity-cli","scratch","hook-debug.log")',
    'try:',
    '    if os.path.exists(dl) and os.path.getsize(dl) > 8*1024*1024:',
    '        os.replace(dl, dl + ".1")',
    'except Exception:',
    '    pass',
