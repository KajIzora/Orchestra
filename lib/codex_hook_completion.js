...
function applyCodexHookCompletion(getState, onEachTask, snapshot, options = {}) {
  ...
  for (const project of getState().projects || []) {
    for (const task of project.tasks || []) {
      ...
      } else if (typeof options.holdDoneForSubagents === 'function' && options.holdDoneForSubagents(snapshot)) {
        // A done-clear (parent Stop) while a sub-agent of this session is still working: defer to
        // the watch poller, which releases at the sub-agent's completion notification (or the quiet
        // backstop). Gate clears above are never held — a sub-agent's gate must surface immediately.
        continue;
      } else if (typeof options.holdDoneForHeartbeat === 'function' && options.holdDoneForHeartbeat(snapshot)) {
        // A done-clear while an ACTIVE near-future self-scheduled heartbeat is pending: the agent
        // yielded to its own wakeup, it did not finish. Defer to the watch poller — the wake's
        // UserPromptSubmit consumes the heartbeat (and re-arms via applyCodexHookResume if a
        // clear slipped through), and the DTSTART + grace bound releases if the wake never comes.
        continue;
      }
      onEachTask(task);
      completedCount += 1;
