...
// The immutable-retry gate for readAgyDbRowsWithImmutableFallback: -wal/-shm presence, checked AT
// RETRY TIME (after the plain read-only open already failed), never before the first attempt.
async function agyDbHasWalSidecar(dbPath) {
  for (const suffix of ['-wal', '-shm']) {
    try {
      await fsp.stat(`${dbPath}${suffix}`);
      return true;
    } catch {
      // absent — keep checking
    }
  }
  return false;
}

// SQLite URI for a lock-free, shm-free read of a checkpointed DB. In the URI path only %, ? and #
// are special; everything else passes through literally (incl. spaces).
function sqliteImmutableReadOnlyUri(dbPath) {
  const encoded = String(dbPath).replace(/%/g, '%25').replace(/\?/g, '%3F').replace(/#/g, '%23');
  return `file:${encoded}?immutable=1`;
}

// Read `sql` from one agy conversation DB read-only, returning the parsed JSON rows. Every local
// conversation-DB reader (step-status AND permission gates) funnels its open through here, so the
// checkpoint-blindness fallback and its invariant are defined and enforced at this ONE point.
//
// Checkpoint-blindness fallback (InFlightAgentFix agy-cli/findings4.md Run A): agy 1.1.1
// checkpoints a GATE-BLOCKED conversation DB ~30 s into the block — WAL contents folded into the
// main file, -wal/-shm sidecars unlinked — while the status-9 gate is still pending. A plain
// `sqlite3 -readonly` open of that sidecar-less WAL-mode DB fails (a read-only connection cannot
// create the missing -shm wal-index ⇒ SQLITE_CANTOPEN), which made cold discovery go dark on a
// gate pending >~30 s. When — and ONLY when — the plain read-only open failed AND the DB has no
// -wal/-shm sidecars (re-checked at retry time), retry via `immutable=1`, which reads without
// locks or a wal-index.
//
// Invariant (hard rule): a DB WITH sidecars is NEVER opened immutable. Sidecars present means a
// live SQLite connection may exist, and immutable=1 ignores the WAL entirely — a stale/torn read
// (the update1.MD §4 Step-4 objection, still correct for that case). Sidecars ABSENT means no live
// WAL writer: the checkpointed main file IS the current state, so the immutable read is neither
// stale nor torn. The residual race (agy re-opens the DB between the sidecar check and the exec)
// is bounded: each call here is one short-lived sqlite3 process, an interleaved write makes that
// one read error (caught by every caller's per-DB skip) or return a vector the next scan corrects.
async function readAgyDbRowsWithImmutableFallback(dbPath, sql) {
  try {
    ...
    if (await agyDbHasWalSidecar(dbPath)) throw err; // live/sidecar-bearing DB: skip, never immutable
    const { stdout } = await execFileAsync(
      SQLITE_BIN,
      ['-readonly', '-json', sqliteImmutableReadOnlyUri(dbPath), sql],
      { timeout: 2500 }
    );
    return parseSqliteJsonRows(stdout);
  }
...
//
// The immutable=1 checkpoint-blindness fallback (readAgyDbRowsWithImmutableFallback above) is
// applied for every caller of this reader, not gated behind a discovery-only option, deliberately:
// both callers of readLocalAgyDbStepStatusSignals re-open per scan and tolerate a frozen
// snapshot-per-open — discoverLocalAgyRuns rebuilds from a FRESH state map each scan and applies
// its own mtime staleness tiers unchanged, and the signal-session poller is mtime-gated so a
// checkpointed DB is read once per mtime bump (the fallback turns its silent post-checkpoint skip
// into the correct status-9 baseline event). agy's next real write recreates the sidecars and
// bumps mtimes, putting the plain -readonly path back in charge.
async function readAgyStepStatusRows(dbPath) {
  const sql = 'SELECT idx, step_type, status FROM steps ORDER BY idx ASC;';
  return readAgyDbRowsWithImmutableFallback(dbPath, sql);
}
...
async function readAgyAppPermissionRows(dbPath) {
  ...
  // Route through the shared read helper so a checkpointed sidecar-less conversation DB (agy
  // checkpoints a gate-blocked conversation ~30 s in — exactly the long-pending gates this reader
  // exists to surface) is read via the immutable=1 fallback instead of going dark; a sidecar-
  // bearing DB whose plain open fails still rethrows and is skipped (invariant lives in the helper).
  return readAgyDbRowsWithImmutableFallback(dbPath, sql);
}
