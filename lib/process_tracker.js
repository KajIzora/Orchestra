...
function stripShellOrnamentArgs(rest) {
  ...
  for (let i = 0; i < 32; i += 1) {
    ...
    // Cursor starts fish terminals with an init command that only sources its
    // shell-integration script. A following script or command remains visible.
    r = r.replace(/^--init-command(?:=|\s+)(?:source\s+)?(?:"[^"]*"|'[^']*'|\S+)(?:\s+|$)/, '');
    r = r.replace(/^--norc(?:\s+|$)/, '');
...
/** Grok CLI process; Grok sessions have their own provider picker. */
function isGrokProcess(command) {
  const tok = argv0Token(command).toLowerCase();
  if (!tok) return false;
  const base = tok.includes('/') ? tok.slice(tok.lastIndexOf('/') + 1) : tok;
  return base === 'grok';
...
/** Optional Orchestra menu-bar helper launched by start.sh. */
function isOrchestraMenubarProcess(proc) {
  if (!proc || !proc.command) return false;
  const base = argv0ShellBasename(proc.command);
  if (!/^python(?:\d+(?:\.\d+)*)?$/.test(base)) return false;
  const rest = argvRest(proc.command);
  const script = rest.replace(/^(["'])(.*)\1$/, '$2');
  return /(?:^|\/)menubar\.py$/.test(script);
}

/** Orchestra's long-lived reverse hook tunnel (remote hooks -> local server). */
function isOrchestraRemoteHookTunnel(proc) {
  if (!proc || !proc.command) return false;
  if (argv0ShellBasename(proc.command) !== 'ssh') return false;
  const command = proc.command;
  return (
    /(?:^|\s)-N(?:\s|$)/.test(command) &&
    /(?:^|\s)-T(?:\s|$)/.test(command) &&
    /(?:^|\s)-S\s+none(?:\s|$)/.test(command) &&
    /(?:^|\s)-o\s+ExitOnForwardFailure=yes(?:\s|$)/.test(command) &&
    /(?:^|\s)-o\s+ServerAliveInterval=30(?:\s|$)/.test(command) &&
    /(?:^|\s)-o\s+ServerAliveCountMax=2(?:\s|$)/.test(command) &&
    /(?:^|\s)-R\s+127\.0\.0\.1:\d+:127\.0\.0\.1:\d+(?:\s|$)/.test(command)
  );
}
...
function looksLikeAggressiveNoise(proc) {
  ...
  if (isOrchestraMenubarProcess(proc)) return true;
  if (isOrchestraRemoteHookTunnel(proc)) return true;
  ...
  if (isGrokProcess(cmd)) return true;
...
/**
 * Orchestra's own remote-poll subprocess. The server fans out short-lived
 * `ssh <host> …` calls (remote hook-log polling, remote `ps`, cwd lookups) roughly
 * once a second, so they flicker in and out of the local process list. Every one is
 * spawned by buildSshArgs() with `-o BatchMode=yes` plus a `ControlMaster=` option —
 * a signature an interactive ssh the user starts themselves never carries. Match on
 * that pair so the user's real ssh sessions still show up.
 * @param {object} proc
 * @returns {boolean}
 */
function isOrchestraOwnSshPoll(proc) {
  if (!proc || !proc.command) return false;
  const base = argv0ShellBasename(proc.command);
  if (base !== 'ssh') return false;
  const lower = proc.command.toLowerCase();
  return lower.includes('batchmode=yes') && lower.includes('controlmaster=');
}
...
function looksLikeNoise(proc, selfPid = null, aggressiveClean = false) {
  ...
  if (isOrchestraOwnSshPoll(proc)) return true;
...
module.exports = {
  ...
  isAgyProcess,
  isGrokProcess,
  isOrchestraOwnSshPoll,
  isOrchestraRemoteHookTunnel,
  isOrchestraMenubarProcess,
};
