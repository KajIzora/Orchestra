...
function normalizeWatchTracking(watchTracking, cursorTracking = null) {
  if (watchTracking && typeof watchTracking === 'object') {
    ...
    if (watchTracking.kind === 'ide_agent') {
      const rawProvider = ['claude', 'claude_cowork', 'codex', 'gemini', 'gemini_cli', 'grok'].includes(watchTracking.provider)
      ...
      if (provider === 'grok' && source === 'ssh') return null; // R2d runs the ssh leg
      ...
      const session_dir = typeof watchTracking.session_dir === 'string' ? watchTracking.session_dir.trim() : '';
      if (!session_id && !transcript_path && !audit_path && !session_dir) return null;
      let remote = null;
      if (source === 'ssh') {
        try {
...
// A "permission" gate is honest ONLY when the provider cannot fold a question into the same
// permission-shaped signal. Cowork records AskUserQuestion and tool-permission BOTH under the
// `permission_request` audit subtype, but the record's `tool_name` distinguishes them, and the
// tracker now classifies by it (claude_cowork_tracker.coworkPermissionRequestGateKind →
// coworkTurnCompletedSince → clear_gate 'question'|'permission'). So a cowork "permission" clear is
// now a REAL tool-permission (an AskUserQuestion clears as 'question') and is trusted — cowork is
// dropped from this fold (Lane D un-fold; gate-distinguishability-matrix.json cowork ⇒
// distinguishable). Browser/terminal still have NO gate content, so their "permission" can't be
// trusted → `unknown`. A "question" clear is always trustworthy (only ever set on positive
// question evidence).
function gateKindReportUntrustworthy(watchTracking) {
  const wt = watchTracking || {};
  if (wt.kind === 'browser_chat') return true;
  return false;
}

// Classify a claude completion hint/snapshot into the internal gate kind. The gated tool's name
// is the discriminator: AskUserQuestion is a question gate (a questions[] payload), everything
// else is a permission gate; attention_reason stays 'permission_request' for BOTH so all
// gate-detection/re-arm logic is untouched — only the reported kind splits.
// SINGLE source of truth for BOTH claude gate-detection paths: the watch poll
// (shouldCompleteIdeAgentWatch below) and the hook-ingest push path (server.js
// applyClaudeHookCompletion). The push path used to hardcode 'permission' and — firing first, at
// PermissionRequest ingest — latched the wrong kind for AskUserQuestion, so
// /api/state watch_finished.gate_kind said 'permission' while the feed said 'question'
// (Phase-3 claude-code findings1 F2, reproduced over ssh).
function claudeGateKindFromHint(hint) {
  return hint && hint.gate_tool_name === 'AskUserQuestion' ? 'question' : 'permission';
}

// Map the internal gate (`wt.clear_gate`) to the gate kind Orchestra REPORTS on watch_finished:
//   'permission' | 'question' | 'unknown' | '' (no gate). Single source of truth shared by
// finishedMarker (the served value) and the lab's Orchestra-state readers (signal_replay /
// orchestra_live_poller) so "what Orchestra reports" is defined in exactly one place.
function reportedGateKind(watchTracking) {
  const gate = watchTracking && watchTracking.clear_gate;
  if (gate === 'question') return 'question';
  if (gate === 'permission') return gateKindReportUntrustworthy(watchTracking) ? 'unknown' : 'permission';
  return '';
}
...
function finishedMarker(watchTracking) {
  ...
  return {
    ...
    // Which KIND of needs-input, when we can honestly tell them apart: 'permission' | 'question' |
    // 'unknown'. Additive — a client that ignores it sees today's generic needs-input pill; '' when
    // this is a plain done. See reportedGateKind for the per-provider honesty policy.
    gate_kind: needsInput ? reportedGateKind(wt) : '',
...
async function shouldResumeIdeAgentWatch(watchTracking, deps = {}, pausedAtMs = 0) {
  ...
  else if (watchTracking.provider === 'grok') fn = deps.shouldResumeGrokWatch;
...
async function finishPausedClaudeWatchIfTerminal(task, watchTracking, deps = {}, pausedAtMs = 0) {
  ...
  // Both gate kinds (a permission prompt AND an AskUserQuestion) can turn out to be a cancel while
  // paused — the claude tracker now marks AskUserQuestion as 'question', so accept both here (was
  // permission-only) or a question gate would lose its cancel detection.
  if (watchTracking.clear_gate !== 'permission' && watchTracking.clear_gate !== 'question') return false;
...
// A paused codex gate whose agent then finished (a terminal Stop, not the next gate's open edge)
...
// finishPausedGeminiWatchIfTerminal; the gate-open guard keeps the *next* gate on the resume path
// rather than mis-finishing it as done.
async function finishPausedCodexWatchIfTerminal(task, watchTracking, deps = {}, pausedAtMs = 0) {
  ...
  // A gate-OPEN completion hint is NOT a terminal. Both codex gate kinds carry a completion_hint so the
  // push path (applyCodexHookCompletion) can open the gate: a PermissionRequest opens a permission gate,
  // and a PreToolUse for request_user_input opens the QUESTION gate. Excluding only PermissionRequest
  // let a question gate's OWN PreToolUse — arriving just after a transcript-detected pause (Channel B
  // leading the Channel A hook) — pass this guard and mis-finish the still-open question as done. That is
  // the intermittent base-codex complex-question false clear: −8s early done clear, a missing continue,
  // and a trailing PostToolUse counted as post-clear activity. Keep any gate-open hint on the resume path.
  const hintIsGateOpen =
    !!hint &&
    (hint.event_name === 'PermissionRequest' ||
      isCodexQuestionGateEvent(hint.event_name, hint.tool_name));
  if (hint && !hintIsGateOpen && hintIsAfterPaused(hint, pausedAtMs)) {
...
async function shouldCompleteIdeAgentWatch(watchTracking, deps = {}) {
  ...
  if (watchTracking.provider === 'claude') {
    ...
    if (hint) {
      ...
      if (!permissionStale) {
        ...
        // Claude stopped because it needs you (permission prompt / AskUserQuestion), vs a plain
        // Stop = finished. claudeGateKindFromHint splits the kind by the gated tool name — shared
        // with the server.js push path so both report the same kind (findings1 F2).
        if (isPermissionAttentionReason(hint.attention_reason)) {
          markHumanGateWatchClear(watchTracking, claudeGateKindFromHint(hint), hint, nowIso);
        }
        return true;
      }
    }
  ...
  if (watchTracking.provider === 'grok') {
    // Same file-based shape as cowork: grok has no hook tap, so the session's LIVE events.jsonl
    // is the whole channel. deps.shouldCompleteGrokWatch = lib/grok_session_tracker.
    // grokTurnCompletedSince → '' | 'done' | 'cancelled' | 'permission' | 'question'
    // ('question' = pending ask_user_question; 'permission' = requested-without-resolved on a
    // non-question tool — the instant always-approve pairs never read as gates).
    const grokFn = deps.shouldCompleteGrokWatch;
    if (typeof grokFn === 'function') {
      const result = await grokFn(watchTracking);
      watchTracking.last_seen_at = nowIso;
      watchTracking.last_error = null;
      if (result === 'permission' || result === 'question') {
        markHumanGateWatchClear(watchTracking, result, null, nowIso);
...
function createWatchPoller(deps) {
  ...
  // Per-task tick errors: log ONCE per task per distinct message (the 1s poll would otherwise
  // storm the log with the same error every tick — never silent-swallow, never a retry storm).
  // Bounded: cleared wholesale past a small cap (deleted tasks would otherwise pin entries).
  const taskTickErrorLogged = new Map();
  function logTaskTickErrorOnce(task, err) {
    const key = String((task && task.id) || '?');
    const msg = err && err.message ? err.message : String(err);
    if (taskTickErrorLogged.get(key) === msg) return;
    if (taskTickErrorLogged.size > 200) taskTickErrorLogged.clear();
    taskTickErrorLogged.set(key, msg);
    console.error(`[watch_tracker] tick: task ${key} watch check failed (isolated; other tasks continue):`, msg);
  }
    ...
    for (const project of state.projects || []) {
      for (const task of project.tasks || []) {
        try {
          ...
          // Completed-watch re-arm (done -> working) for the FILE-BASED ide_agent providers:
          // claude_cowork (audit.jsonl) and grok (session events.jsonl). Neither has hooks, so
          // unlike claude/codex/cursor — whose FINISHED watch is re-armed by a hook POST
          // (applyClaude/Codex/CursorHookResume) the instant a new prompt arrives — a followed-up
          // turn is only observable by polling the session's own file. When a finished (done,
          // non-gate, non-cancel) watch's file shows a FRESH generation burst that started after
          // the done, re-arm the SAME watch and let the completion pass track the follow-up turn
          // to its own done. Uses the exact active-generation classifier the paused-gate resume
          // already relies on (deps.shouldResumeClaudeCoworkWatch / deps.shouldResumeGrokWatch);
          // generatedAfter() guards against re-detecting the seed.
          const completedRearmFn =
            task.completed_watch_tracking && task.completed_watch_tracking.kind === 'ide_agent'
              ? task.completed_watch_tracking.provider === 'claude_cowork'
                ? deps.shouldResumeClaudeCoworkWatch
                : task.completed_watch_tracking.provider === 'grok'
                  ? deps.shouldResumeGrokWatch
                  : null
              : null;
          if (
            task.status === 'todo'
            && task.watch_finished
            && !task.watch_finished.needs_input
            && !task.paused_watch_tracking
            && task.completed_watch_tracking
            && !task.completed_watch_tracking.clear_cancelled
            && typeof completedRearmFn === 'function'
          ) {
            const cwt = normalizeWatchTracking(task.completed_watch_tracking, null);
            if (cwt) {
              const doneAtMs = Date.parse(task.watch_finished.at || cwt.clear_signal_at || '') || 0;
              let gen = null;
              try {
                gen = await completedRearmFn(cwt);
              } catch {
                gen = null; // transient file read error: keep the watch finished, retry next tick
              }
              if (generatedAfter(gen, doneAtMs)) {
                const wt = { ...cwt };
                resumeWatchTracking(task, wt, deps.applyTaskStatusChange);
                // Anchor the re-armed completion check at the follow-up's generation start (strictly
                // after the seed's done) so the seed's result is out of scope and the next result the
                // completion pass sees is the follow-up's — the *TurnCompletedSince scans drop
                // ts < linked_at.
                const startMs = Date.parse(gen.start_signal_at || '') || 0;
                if (startMs > doneAtMs) wt.linked_at = new Date(startMs).toISOString();
                changed = true;
              }
            }
            continue;
          ...
          // Per-task isolation (Phase-4 P4; verify-lane findings1 §2): ONE task whose per-task
          // probe throws (e.g. a disallowed transcript path via shouldCompleteCodexWatch /
          // shouldResumeIdeAgentWatch) used to abort the ENTIRE tick here, stalling every task
          // behind it in the iteration order. Contain it to the task; the next tick retries.
          logTaskTickErrorOnce(task, err);
        }
      }
    }
