    ...
    for (const project of state.projects || []) {
      for (const task of project.tasks || []) {
        ...
        if (wt.kind === 'cursor') {
          ...
          if (typeof transcriptFn === 'function') {
          ...
          // AUGMENT: cursor's transcript AskQuestion is delayed (written only after the answer), so
          // also consult the chat store.db, which holds the pending question the instant the gate
          // renders (lib/cursor_chat_db.js, local-only). Either source flips the question gate.
          if (!askQuestionClear && typeof deps.getCursorChatDbQuestionHint === 'function') {
