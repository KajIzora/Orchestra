...
const STREAM_SIGNAL_MARKERS = new Set([
  ...
  'end_of_stream',
  'conversation_id',
  // Realtime-WS content heartbeat: a structural-only `stream_delta` the spoofer emits (throttled)
  // while ChatGPT streams a handoff turn's answer over the realtime WebSocket. It carries NO content
  // and is INERT to production done/resume (see TERMINAL_STREAM_MARKERS + applyBrowserChatResume); it
  // exists purely so the test harness's activity-quiescence GT can see the WS stream is still alive.
  'stream_delta',
...
function createBrowserChatStore(options = {}) {
  ...
  // Opt-in assistant-TEXT stream (FollowUps §3.3), keyed by `${provider}:${cid}`. Unlike the
  // structural signals above, this DOES hold model message content — populated only from the
  // dedicated /stream-body content endpoint, which the extension only posts to under the explicit
  // body-streaming opt-in. Exactly ONE current message per conversation (the live-feed adapter
  // renders it as a single evolving note), with a monotonic seq so the adapter can detect updates.
  /** @type {Map<string, object>} */
  const streamBodyByKey = new Map();
  let streamBodySeq = 0;
  ...
  // Ingest one assistant-text snapshot (§3.3). Keeps ONE current message per conversation. Text
  // grows monotonically within a turn, so within a turn we keep the LONGER of stored/incoming (a
  // late out-of-order or dual-source — sniffer + DOM — snapshot can't shorten the note). A new turn
  // is recognized when the turn_id changes, or when a non-final snapshot follows a completed
  // (final) one — then we reset outright. `final` marks the last snapshot of a turn.
  function ingestStreamBody(body) {
    if (!body || typeof body !== 'object') return { ok: false, error: 'body required' };
    const provider = normalizeProvider(body.provider);
    if (!provider) return { ok: false, error: 'provider must be chatgpt, claude, or gemini' };
    const cid = normalizeConversationId(body.conversation_id);
    if (!cid) return { ok: false, error: 'conversation_id required' };
    const incomingText = typeof body.text === 'string' ? body.text : '';
    const final = body.final === true;
    const turn_id = typeof body.turn_id === 'string' ? body.turn_id.trim().toLowerCase() : '';
    const source = body.source === 'dom' ? 'dom' : 'stream';
    const key = `${provider}:${cid}`;
    const prev = streamBodyByKey.get(key);
    let text = incomingText;
    let isFinal = final;
    let turnId = turn_id;
    // turn_seq advances ONLY when a new turn begins (turn_id change, or a non-final snapshot after a
    // completed one). It stays constant while a turn's text grows — the live-feed adapter keys its
    // prompt/note reset on it (turn_id alone can't: the DOM/gemini path carries no turn_id).
    let turnSeq = prev ? prev.turn_seq : 1;
    if (prev) {
      const turnChanged = turn_id && prev.turn_id && turn_id !== prev.turn_id;
      const newTurnAfterFinal = prev.final && !final;
      if (turnChanged || newTurnAfterFinal) {
        turnSeq = prev.turn_seq + 1;
      } else {
        // Same turn: never let a shorter snapshot clobber a longer one, unless it's a nonempty final
        // (the authoritative full message).
        if (incomingText.length < prev.text.length && !(final && incomingText.length > 0)) {
          text = prev.text;
        }
        isFinal = final || prev.final;
        turnId = turn_id || prev.turn_id;
      }
    }
    const entry = {
      provider,
    ...
    streamBodyByKey.set(key, entry);
    return { ok: true, body: entry };
  }

  // Latest assistant-text snapshot for a conversation, or null. Read by the live-feed browser
  // stream adapter (lib/browser_stream_adapter.js).
  function latestStreamBody(providerFilter, conversationId) {
    const want = normalizeProvider(providerFilter);
    const cid = normalizeConversationId(conversationId);
    if (!want || !cid) return null;
    return streamBodyByKey.get(`${want}:${cid}`) || null;
  }

  // Drop all retained assistant-text (every conversation). Called alongside clearStreamSignals so a
  // fresh capture wave never inherits a prior run's message text.
  function clearStreamBody() {
    streamBodyByKey.clear();
  }
...
/**
 * Mark the watch_tracking of any task watching this conversation as stream-opted-in (§3.3). This is
 * the recorded LINKAGE fact the live-feed tier bump keys on (liveTierForWatch) — exactly like cowork
 * keying tier on `audit_path`. Idempotent; returns the number of tasks NEWLY marked so the caller
 * can gate its storage.save().
 * @returns {number}
 */
function markBrowserChatStreamOptin(getState, provider, conversation_id) {
  const p = normalizeProvider(provider);
  const cid = normalizeConversationId(conversation_id);
  if (!p || !cid) return 0;
  let marked = 0;
  for (const project of getState().projects || []) {
    for (const task of project.tasks || []) {
      ...
      if (wt.provider !== p) continue;
      if (normalizeConversationId(wt.conversation_id) !== cid) continue;
      if (wt.stream_optin === true) continue;
      wt.stream_optin = true;
      marked += 1;
    }
  }
  return marked;
...
function applyBrowserChatResume(getState, onResumeTask, provider, conversation_id, opts = {}) {
  ...
  // A `stream_delta` is a mid-turn content heartbeat (realtime WS), not a new turn — it exists only
  // to feed the test harness's quiescence GT and must stay inert to the done→working resume path.
  if (marker === 'stream_delta') return 0;
  let resumed = 0;
  for (const project of getState().projects || []) {
    for (const task of project.tasks || []) {
      if (task.status === 'waiting') continue;
