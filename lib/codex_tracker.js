...
const REMOTE_CODEX_SESSION_INDEX = '$HOME/.codex/session_index.jsonl';
...
// Codex records a session id in three shapes: the lifecycle hooks and the rollout's session_meta
// carry the bare session UUID (019f…-…); the rollout transcript FILE is named
// rollout-<timestamp>-<uuid>.jsonl. The on-disk thread index (~/.codex/session_index.jsonl) is
// keyed by the bare UUID. Normalize any of those forms to the bare UUID so a title lookup matches
// regardless of which the caller holds. When no UUID is present (older or synthetic ids) fall back
// to the trimmed value so an exact-id index still matches.
const CODEX_SESSION_UUID_RE =
  /([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})(?:\.jsonl)?$/i;
function extractCodexSessionKey(value) {
  const s = typeof value === 'string' ? value.trim() : '';
  if (!s) return '';
  const m = s.match(CODEX_SESSION_UUID_RE);
  return m ? m[1].toLowerCase() : s;
}

// mtime-cached reader for the Codex thread index. The picker enriches up to DEFAULT_MAX_RUNS rows on
// every refresh; re-reading and re-parsing session_index.jsonl per row would be wasteful, so we cache
// the parsed Map<sessionKey,{title,updated_at}> and reuse it until the file's mtime/size changes.
// Never throws — a missing or unreadable index yields an empty Map (callers then return '').
let codexSessionIndexCache = { path: '', mtimeMs: -1, size: -1, map: new Map() };
function loadCodexSessionIndexMap(homeDir = os.homedir()) {
  const indexPath = path.join(getCodexRoot(homeDir), 'session_index.jsonl');
  let st;
  try {
    st = fs.statSync(indexPath);
  } catch {
    return new Map();
  }
  if (
    codexSessionIndexCache.path === indexPath &&
    codexSessionIndexCache.mtimeMs === st.mtimeMs &&
    codexSessionIndexCache.size === st.size
  ) {
    return codexSessionIndexCache.map;
  }
  const map = new Map();
  try {
    for (const row of parseCodexSessionIndex(indexPath)) {
      const key = extractCodexSessionKey(row.session_id);
      if (!key) continue;
      // A session id should appear once, but if it repeats keep the newest updated_at.
      const prev = map.get(key);
      if (!prev || String(row.updated_at || '') >= String(prev.updated_at || '')) {
        map.set(key, { title: row.title || '', updated_at: row.updated_at || '' });
      }
    }
  } catch {
    return codexSessionIndexCache.map || new Map();
  }
  codexSessionIndexCache = { path: indexPath, mtimeMs: st.mtimeMs, size: st.size, map };
  return map;
}

// Real Codex thread title for a picker run, read from the on-disk thread index that Codex writes
// when a thread is named (the desktop app, the IDE/plugin, and the interactive TUI all populate
// thread_name). Keyed by the run's session UUID — taken from run.session_id (hooks + session_meta
// carry the bare UUID) and, as a fallback, from the rollout filename which embeds the same UUID.
// Returns '' when the session has no named-thread entry: `codex exec` and app-server-driven runs are
// never auto-named, and we must NOT fabricate a title from the first user prompt. Never throws.
function codexThreadTitleForRun(run, homeDir = os.homedir()) {
  try {
    if (!run || typeof run !== 'object') return '';
    const key =
      extractCodexSessionKey(run.session_id) ||
      extractCodexSessionKey(
        typeof run.transcript_path === 'string' ? path.basename(run.transcript_path) : ''
      );
    if (!key) return '';
    const hit = loadCodexSessionIndexMap(homeDir).get(key);
...
// Remote (ssh) codex thread-title cache: `${host}:${sessionKey}` -> { title, checkedMs }. The
// `thread_name` lives in the REMOTE ~/.codex/session_index.jsonl, which the local index reader above
// can't see, so grep the one matching line over ssh (keyed by the session UUID). Found titles are
// immutable → cached forever; a miss is re-checked at most every REMOTE_TITLE_RECHECK_MS so a
// not-yet-named thread can gain a title without an ssh round-trip on every poll. Never throws.
const remoteCodexTitleCache = new Map();
const REMOTE_CODEX_TITLE_CACHE_MAX = 512;

async function fetchRemoteCodexThreadTitle(sessionKey, remote, runSsh, timeoutMs) {
  try {
    if (!sessionKey) return '';
    const cfg = assertValidRemoteSource(remote);
    const key = `${cfg.host}:${sessionKey}`;
    const cached = remoteCodexTitleCache.get(key);
    ...
    const idx = REMOTE_CODEX_SESSION_INDEX;
    const cmd = `if [ -f ${idx} ]; then grep -F -- ${shellQuote(sessionKey)} ${idx} 2>/dev/null | tail -n 1 || true; fi`;
    const raw = await ssh(cfg.host, cmd, timeoutMs);
    const line = String(raw || '').trim().split('\n').filter(Boolean).pop() || '';
    let title = '';
    if (line) {
      try {
        const obj = JSON.parse(line);
        // Confirm the grep hit is the id field (not an incidental substring match).
        if (obj && extractCodexSessionKey(obj.id) === sessionKey && typeof obj.thread_name === 'string') {
          title = obj.thread_name.trim();
        }
      } catch {
        title = '';
      }
    }
    if (remoteCodexTitleCache.size >= REMOTE_CODEX_TITLE_CACHE_MAX) remoteCodexTitleCache.clear();
    remoteCodexTitleCache.set(key, { title, checkedMs: Date.now() });
    return title;
  } catch {
    return '';
  }
...
// Codex records the surface in the rollout's `session_meta` (first line): `originator` +
// `source`. Map to Orchestra's surface kinds: the VS Code / Cursor extension → 'plugin', the
// desktop app → 'desktop', and the interactive TUI / `codex exec` → 'cli'.
function normalizeCodexSurface(originator, source) {
  const o = String(originator || '').trim().toLowerCase();
  const s = String(source || '').trim().toLowerCase();
  if (o === 'codex_vscode' || s === 'vscode') return 'plugin';
  if (o.includes('desktop')) return 'desktop'; // "Codex Desktop"
  if (o === 'codex-tui' || o === 'codex_exec' || s === 'cli' || s === 'exec') return 'cli';
  return '';
}

function extractCodexSessionMeta(tailText) {
  const out = { workspace_path: '', surface: '' };
  ...
  for (const line of tailText.split('\n')) {
    ...
    out.surface = normalizeCodexSurface(obj.payload?.originator, obj.payload?.source);
    break;
  }
  return out;
}

// session_meta is the FIRST line of a rollout, so the 512KB tail read misses it on long sessions.
// Read the head (session_meta carries base_instructions, so allow generous room) to recover the
// surface. Sync I/O, matching the tail reader; only a handful of picker rows are enriched per poll.
const CODEX_META_HEAD_BYTES = 256 * 1024;

function codexSurfaceFromTranscriptHead(resolvedPath) {
  let fd;
  try {
    fd = fs.openSync(resolvedPath, 'r');
    const size = fs.fstatSync(fd).size;
    const len = Math.min(CODEX_META_HEAD_BYTES, size);
    if (len <= 0) return '';
    const buf = Buffer.alloc(len);
    fs.readSync(fd, buf, 0, len, 0);
    return extractCodexSessionMeta(buf.toString('utf8')).surface || '';
  } catch {
    return '';
  } finally {
    if (fd !== undefined) {
...
async function discoverCodexRuns(homeDir = os.homedir(), options = {}) {
  ...
  // Optional mtime prefilter (options.recentOnlyMs): statSync is cheap, tailing 512KB per rollout
  // is not, and ~/.codex/sessions holds ~1000 rollouts. With a recency window set, only rollouts
  // touched within it survive to be tailed/classified — a generating run necessarily has a recent
  // mtime, and applyActiveGenerationStaleCutoff would flip anything older to stale anyway. Without
  // the option, behavior is unchanged. (The sort+slice to maxRuns below already bounds the tail
  // either way; the prefilter just avoids ranking ~1000 stale rollouts on every picker poll.)
  const recentOnlyMs =
    Number.isFinite(options.recentOnlyMs) && options.recentOnlyMs > 0 ? options.recentOnlyMs : 0;
  const nowMs = Number.isFinite(options.nowMs) ? options.nowMs : Date.now();
  // Keyed by the normalized session UUID; the rollout filename basename embeds the same UUID.
  const indexById = loadCodexSessionIndexMap(homeDir);
  ...
  for (const transcriptPath of files) {
    ...
    const idx = indexById.get(extractCodexSessionKey(sessionId));
    runs.push({
      kind: 'ide_agent',
      provider: 'codex',
      source: 'local',
...
async function enrichCodexPickerRunWithTranscript(run, options = {}) {
  ...
  const out = { ...run };
  // Backfill the run's real thread title from the on-disk thread index when the hook-store row has
  // none (hook snapshots never carry a title). Real titles only — codexThreadTitleForRun returns ''
  // when the session was never named, so a `codex exec` / app-server run keeps title ''. Done before
  // the transcript read so a run with no readable transcript still gets its title.
  if (!out.title) {
    if (run.source === 'ssh' && options.remote) {
      // The thread_name lives in the REMOTE session_index.jsonl; the local index reader can't see it.
      const key =
        extractCodexSessionKey(out.session_id) ||
        extractCodexSessionKey(
          typeof out.transcript_path === 'string' ? path.basename(out.transcript_path) : ''
        );
      out.title = await fetchRemoteCodexThreadTitle(
        key,
        options.remote,
        options.runSsh,
        options.timeoutMs
      ...
      const title = codexThreadTitleForRun(out, options.homeDir);
  ...
  try {
    ...
    if (run.source === 'ssh' && options.remote) {
      ...
      // The surface lives in the first-line session_meta, which the tail read below misses.
      if (!out.surface) {
        const surf = codexSurfaceFromTranscriptHead(resolved);
