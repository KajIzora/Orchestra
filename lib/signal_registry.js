 ...
 * and the generated signal-map doc (docs/SignalSessionRunbook/signal-map.md) all read.
 * Adding a platform starts with an entry here.
 *
...
const REGISTRY = {
  claude: {
    ...
    hookCatalog: {
      ...
      //
      // PreToolUse / MessageDisplay / PostToolUseFailure are captured for the LIVE FEED (Phase 2b):
      // PreToolUse → tool_start (the register "now" line), MessageDisplay → note (mid-turn assistant
      // text), PostToolUseFailure → tool_end ok:false. Like PostToolUse they are pure activity pings
      // in the claude hook store (shouldSetCompletionHint / isPermissionCompletionEvent /
      // attentionReasonFromEvent all return falsy for them — they never fake a done or a gate); their
      // real payloads are read from the raw hook_event_log tap by lib/live_turn_normalizer.js. Both
      // the local and remote installers derive the subscribed set from getHookEventsForProfile('claude',
      // 'production') == this array, so SSH stays in lockstep (hooks are `same` over the tunnel). ~150
      // B/s per active agent (LiveFeedDataRequirements.MD; the maximal remote profile already carried
      // MessageDisplay at 434 B/s in the Lane B ssh smoke).
      captured: [
        'UserPromptSubmit',
        'SessionStart',
        'Stop',
    ...
    sources: [
      ...
      },
      {
        ...
        via: 'transcript',
        detail:
          'CORROBORATING channel (additive): the session JSONL carries the done outcome as CONTENT — terminal result rows readable per session file (the same predicate the ssh completion read shouldCompleteRemoteClaudeWatch and replay already run). Evidence: live campaign 2026-07-10 d1-full-live, content-confirmed — transcripts content-confirmed done+permission and done+question, attributed by conversation_id/run_id + sentinel keys. Production PRIMARY remains the Stop hook above (transcript end_turn alone does NOT clear locally); this row records the transcript as a corroborating done source.',
      },
      {
        outcome: 'permission',
        via: 'hook',
      ...
      },
      {
        ...
        via: 'transcript',
        detail:
          'CORROBORATING channel (additive): the pending permission gate is readable as CONTENT in the session JSONL (the gated tool_use awaiting its user tool_result — the read backing the permission-hint stale check). Evidence: live campaign 2026-07-10 d1-full-live, content-confirmed (done+permission in the permission run\'s transcript, conversation_id/sentinel-keyed). Production PRIMARY remains the PermissionRequest hook above; this transcript channel corroborates.',
      },
      {
        outcome: 'question',
        via: 'hook',
      ...
      },
      {
        ...
        via: 'transcript',
        detail:
          'CORROBORATING channel (additive): a pending AskUserQuestion tool_use is readable as CONTENT in the session JSONL. Evidence: live campaign 2026-07-10 d1-full-live, content-confirmed (done+question in the question run\'s transcript, conversation_id/sentinel-keyed). Production PRIMARY remains the hook above (AskUserQuestion surfaces as PermissionRequest, gate=permission); this transcript channel corroborates.',
      },
      {
        outcome: 'resume',
        via: 'hook',
  ...
  cursor_ide: {
    ...
    hookCatalog: {
      // Forwarded to the live app (these are what ensureLocalCursorHooksInstalled installs; this is
      // the SINGLE ~/.cursor/hooks.json that BOTH cursor-cli and the Cursor IDE read —
      // getHookEventsForProfile('cursor') resolves here for both).
      ...
      // Phase-2b live-feed (cursor-cli): afterAgentResponse/-Thought (notes, mid-turn narrative)
      // and afterFileEdit (a file-edit register end) were promoted from `available` → the
      // production profile so cursor-cli EMITS them into hook_event_log and the live feed reaches
      // T2. afterAgentResponse/-Thought are conversation-lifecycle hooks (fire reliably even when
      // the per-tool hooks don't — see the CLI catalog caveat below); afterFileEdit is a per-tool
      // hook (best-effort). cursor-ide passively inherits the SAME hooks.json (shared artifact); no
      // IDE-side change is required and none is made — see docs cursor-2b-notes.md §cursor-ide.
      captured: [
        'beforeSubmitPrompt',
        'stop',
        'sessionStart',
  ...
  codex: {
    ...
    // Codex's thread/session catalog DB (threads registry + thread_spawn_edges sub-agent graph +
    // agent_jobs). Session-attributable by CONTENT (threads.id / rollout_path = the codex
    // conversation id — the SAME key the wired `hooks` source uses and that `rollout_path` points
    // the wired `transcript` at); title/first_user_message/preview are verbatim prompt = PROMPT-ECHO.
    // It is NOT a tracking signal: the row has NO status/waiting/gate column — it is written as a
    // progress heartbeat (updated_at + tokens_used + preview) across the whole turn, with a final
    // write at done; a write may coincide with a gate window but carries no gate-distinguishing
    // content (updated_at bumps; tokens freeze during the pause — an absence, not a marker), so it
    // cannot distinguish generating/permission/question/done/cancelled. Recorded as EXTRA (ignored
    // on replay) — a capture-everything context/metadata catalog, dominated by the wired transcript
    // it merely indexes. Closure Campaign L7 probe 2026-07-12 (codex-cli 0.142.0), resolving the
    // d2-codex-recheck follow-up; the agy-app analogue is `~/.gemini/antigravity/conversations/
    // <conversation>.db`. remote:'skip' (extraSource seed default; NO ssh validation — global
    // no-ssh rule; a future ssh round could flip it to remote-read like the transcript).
    extraSources: ['~/.codex/state_5.sqlite (+ -wal/-shm)'],
    liveOnlySources: [],
    recordedChannels: ['hook_events', 'transcript_phases'],
    replayChannels: ['hook_events', 'transcript_phases'],
    hookCatalog: {
    ...
    sources: [
      ...
      { outcome: 'question', via: 'transcript', detail: 'pending request_user_input in transcript; combined detection prefers this over the turn-complete hook' },
      {
        ...
        via: 'hook',
        detail:
          'CORROBORATING channel (additive): the hook stream demonstrably carries the question gate as CONTENT — the request_user_input tool fires PreToolUse/PostToolUse hook events (tool_name "request_user_input", tool_input.questions[…]) under the parent session_id. Evidence: live campaign 2026-07-10 d2-codex-recheck, content-confirmed — the harvest classified hook:codex as carrying question across scenarios (the question run\'s gate emitted both events, session-attributed). Production detection is UNCHANGED: combined detection still prefers the transcript\'s pending request_user_input (row above), because the turn-complete Stop hook alone misreads a question end as done (see note); this row records that the hook channel is a real, content-attributable question source too.',
      },
