...
const REGISTRY = {
  ...
  cursor_ide: {
    ...
    ssh: {
      ...
      note:
        'Two distinct ssh paths share this surface. (1) Orchestra\'s LIVE remote-cursor-workspace watch — the location/transcriptRead/differences above (server reads the remote transcript over ssh; renderer/exec stay local). (2) The signal-lab `--source ssh` HARNESS for cursor-ide — Phase 1: the Cursor GUI + AX driver stay LOCAL (watch-only, no remote launch) and the remote agent\'s hooks reach Orchestra via the reverse tunnel, so done/clear is driven by the remote stop hook. The recorder\'s single mixed `cursor_probe` source runs identically over ssh (capture parity: cursor_probe=same); capturing the remote transcript + renderer/exec gate sources is Phase 2 (it splits cursor_probe into granular remote-read sources).',
    },
    note: 'renderer.log fires user-approval-requested for main-agent permissions/questions but is silent for sub-agent shell permissions. Cursor Agent Exec.log fires for all shell permissions regardless of agent level and is used as the universal source; renderer.log still handles question gates and is needed to resolve window→composerId for the exec probe. Both probes run in parallel — for main-agent shell permissions both fire (deduped by pendingByConversation); for sub-agent permissions only the exec probe fires. signal-bank Cursor IDE recordings use provider `cursor_ide` so replay and bank paths distinguish the IDE from cursor-agent CLI; live Orchestra watch_tracking still uses the production `cursor` key.',
  },
  cursor_cli: {
    status: 'supported',
    provider: 'cursor_cli',
    ...
    liveOnlySources: [],
    recordedChannels: ['hook_events', 'transcript_phases', 'chat_db_phases'],
    replayChannels: ['hook_events', 'transcript_phases', 'chat_db_phases'],
    hookCatalog: {
    ...
    knownGaps: [
      {
        ...
        summary:
          'A background sub-agent that keeps working after the parent turn Stops causes a FALSE CLEAR: the watch clears to done on the parent\'s Stop while the sub-agent is still running.',
        detail:
          'Repro: a prompt like "spawn a sub agent and reply when it is up and running" — the coordinator (top-level turn) emits its Stop ~as soon as the sub-agent is launched, so the watch clears, but the sub-agent (a separate turn_id under the SAME session_id) keeps firing PreToolUse/PostToolUse for tens of seconds afterward. Verified in signal-lab/codex-desktop/2026-06-24T08-27-37-070Z (desktop) and signal-lab/codex/2026-06-24T08-42-30-146Z (api). ' +
          'Why it is NOT cleanly fixable with today\'s signals: sub-agent turns emit NO Stop hook, and SubagentStop (listed in hookCatalog.available) is NOT fired by codex\'s collab/spawn_agent feature (zero SubagentStart/Stop POSTs across every captured run). The sub-agent\'s task_complete lands in a SEPARATE rollout the parent watcher never reads, and codex_turn_completed is an app-server JSON-RPC event the signal-lab HARNESS sees but the LIVE product (hooks + parent transcript only) does not. Net: the live watcher CAN see the sub-agent START (its hooks carry the parent session_id with a new turn_id) but has no clean END signal — so it currently clears on the parent Stop.',
        candidateFix:
          'Idea 1 (turn accounting): do not clear on the parent Stop while a nested sub-agent turn has opened (a 2nd turn_id seen before the parent Stop). Idea 2 (settle): after the parent Stop, an ~8s confirm window — no activity -> clear (parent waited then stopped, the common case); activity -> escalate to a ~15s quiescence drain (must exceed the sub-agent\'s own sleep gaps, measured up to ~10.5s) and clear once quiet. TRADEOFF (why deferred): the confirm window also lands on the common parent-waits-then-stops runs (indistinguishable from the false-clear case at the instant of the Stop — the false-clear case\'s first post-stop hook is ~3.8s later), adding ~8s latency to basic-sub-agent / parallel-sub-agent / robust-sub-agent, which would regress their <3s latency grade unless that threshold is raised. Cleaner alternatives that avoid the latency hit: (a) get SubagentStop to actually fire (then "clear once all turns have Stopped" works exactly), or (b) treat the parent\'s wait_agent PostToolUse completing before its Stop as proof the sub-agents are done (skip the confirm).',
        status: 'not-implemented — maintainer decision pending (false-clear vs sub-agent-run latency tradeoff)',
      },
    ],
    replayModes: ['both', 'hooks-only', 'transcript-only'],
    ssh: {
...
const SSH_SOURCE_DISPOSITIONS = {
  ...
  // cursor_ide --source ssh (Phase 1): the Cursor GUI + AX driver stay LOCAL (watch-only, no remote
  // launch); only the agent + its signals are remote. Two sources:
  ...
  //     Agent-Exec.log gate signals, which are local-by-nature even for a Remote-SSH workspace
  //     (the Cursor UI renders on the control machine), so they stay local on purpose.
  ...
  //     remote host and is read over ssh (_pollTranscriptRemote). renderer/exec gates remain local.
  cursor_ide: {
    cursor_probe: 'same',
    transcript: 'remote-read',
  },
