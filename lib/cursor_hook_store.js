...
// Upper bound for holding a parent picker row open on a still-running sub-agent — mirrors
// watch_tracker's CURSOR_CONTINUATION_HOLD_MAX_MS so a wedged child (subagentStop never fires,
// which the IDE's unreliable hook can cause) cannot pin the row visible forever.
const CURSOR_PICKER_SUBAGENT_HOLD_MAX_MS = (() => {
...
function createCursorHookStore(options = {}) {
  ...
  // True when a parent conversation still has a running sub-agent: cursor fires `stop` once per
  // GENERATION, so a parent that spawned a Task sub-agent goes terminal on its own stop while the
  // child keeps working (its subagent:<id> snapshot stays non-terminal until subagentStop). The
  // watch holds across that boundary (watch_tracker cursorCompletedHintContinuationGate); this is
  // the snapshot-derivable proxy so the parent picker row stays visible instead of vanishing.
  // Bounded by CURSOR_PICKER_SUBAGENT_HOLD_MAX_MS so a wedged child cannot pin it forever.
  function parentHasOpenSubagent(parentConversationId, nowMs) {
    const parent = normalizeConversationId(parentConversationId);
    if (!parent) return false;
    for (const snap of byKey.values()) {
      if (!String(snap.snapshot_key || '').startsWith('subagent:')) continue;
      if (normalizeConversationId(snap.parent_conversation_id) !== parent) continue;
      if (snap.terminal_hint) continue; // subagentStop seen → child closed
      if (snap.generating !== true) continue; // not actively running
      const ageMs = nowMs - (Date.parse(snap.updated_at || '') || 0);
      if (ageMs > CURSOR_PICKER_SUBAGENT_HOLD_MAX_MS) continue; // escape: wedged child
      return true;
    }
    return false;
  }
  ...
  function listRunsForProject(project, options = {}) {
    ...
    for (const snap of listSnapshots()) {
      if (String(snap.snapshot_key || '').startsWith('subagent:')) continue;
      // A parent whose watch is held on a still-running sub-agent stays visible even though its
      // own stop made the snapshot terminal (generating:false / terminal_hint:true).
      const held = parentHasOpenSubagent(snap.conversation_id, nowMs);
      if (activeOnly && snap.generating !== true && !held) continue;
      ...
      out.push({
        ...
        surface: cursorSurfaceFromVersion(snap.cursor_version),
        hook_hint: true,
