...
function snapshotToClaudePickerRun(snap, source, remote, workspace) {
  ...
  return {
    ...
    mtime_ms: updatedMs,
    // Only ever a real human-prompt preview, else empty — never the session id. The client
    // label chain is `last_user_preview || title || session_id`, so seeding the UUID here
    // would mask the real chat title whenever the hook carried no prompt (long tool-heavy
    // plugin chats). Let the client fall through to the title, and to the id only as a last resort.
    last_user_preview: snap.last_user_preview || '',
...
// The local sibling of the ssh fallback above, with two deliberate divergences:
// - ALWAYS merged, not gated on zero snapshot rows: hook snapshots are in-memory only, so an
//   agent that started while Orchestra was down has no snapshot — and once Orchestra is up,
//   snapshots from OTHER local runs arrive, so a zero-rows gate would re-hide the missed run the
//   instant any one agent fires a hook.
// - only rows the artifact classifies as generating are merged: done/stale local history is the
//   hook store's business, discovery exists purely to surface in-flight runs the hooks missed.

/**
 * Merge locally-discovered transcript runs (discoverClaudeRuns output, ideally bounded with
 ...
 *   tell them apart (host/projects_root null, state_location '', completion_hint false,
 *   attention_reason '') — plus discovered:true so live tests can;
 * - dedupes by transcript path / session id via the shared core (a snapshot row always wins).
 */
function mergeLocalClaudeDiscoveryPickerRuns(snapshotRuns, discoveredRuns, opts = {}) {
  const localWorkspaces = Array.isArray(opts.localWorkspaces)
    ? opts.localWorkspaces.map((p) => String(p || '').trim()).filter(Boolean)
    : [];
  const candidates = [];
