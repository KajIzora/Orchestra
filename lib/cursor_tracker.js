...
/**
 * Newest mtime among a run's sub-agent child transcripts (cheap: readdir + stat, no tailing).
 * Cursor writes Task/background sub-agent activity to <runId>/subagents/<childId>.jsonl; a parent
 * whose current activity is child-only has an idle parent .jsonl (stale mtime, turn_ended tail) and
 * would otherwise look done. Missing/unreadable subagents dir (the common case) folds to nothing.
 * @returns {{ mtimeMs: number, path: string }}
 */
function newestCursorSubagentChild(runDir) {
  const subagentsDir = path.join(runDir, 'subagents');
  let entries;
  try {
    entries = fs.readdirSync(subagentsDir, { withFileTypes: true });
  } catch {
    return { mtimeMs: 0, path: '' };
  }
  let best = { mtimeMs: 0, path: '' };
  for (const entry of entries) {
    if (!entry.isFile() || !entry.name.endsWith('.jsonl')) continue;
    const childPath = path.join(subagentsDir, entry.name);
    let st;
    try {
    ...
    if ((st.mtimeMs || 0) > best.mtimeMs) best = { mtimeMs: st.mtimeMs || 0, path: childPath };
  }
  return best;
}

/**
 * Discover local Cursor runs for the picker, CLASSIFIED for in-flight generation (cold-start fix).
 *
 * Unlike discoverCursorRuns (which only lists runs, no `generating` field), this reads each recent
 * transcript's tail and stamps `generating` so the picker can surface a run whose start hook was
 * missed (Orchestra was down) yet DROP finished/stale ones (the false-positive guard the live hook
 * path never needed). Classification per run:
 *  - parent tail is `turn_ended` (any status: completed/aborted/cancelled/error, incl. the manual
 *    abort variants) ⇒ the parent's own turn is over. A tail that is NOT turn_ended (or an
 *    unparseable partial write ⇒ unknown) ⇒ the parent turn is open;
 *  - sub-agent blind spot: a parent whose activity is currently child-only — its own turn ended but
 *    a <runId>/subagents/*.jsonl child is fresh and non-terminal — stays generating (`held`). This
 *    is the disk analogue of the live store's parentHasOpenSubagent/held (cursor_hook_store.js);
 *  - last-activity is max(parent mtime, newest child mtime), then applyActiveGenerationStaleCutoff
 *    flips anything quiet for longer than the stale window to generating:false — a run killed
 *    mid-turn never writes turn_ended and would otherwise read generating forever.
 *
 * Bounded like discoverClaudeRuns' recentOnlyMs: a stat-only prefilter (folding child mtimes) keeps
 * the scan to runs touched within options.recentOnlyMs BEFORE any tail read, then only the newest
 * maxRuns are tailed. Fail-safe: unreadable dirs/files contribute nothing and never throw; numeric
 * IDE workspace slugs whose agent-transcripts dir is empty/absent are skipped.
 *
 * @returns {Promise<Array<{ run_id: string, transcript_path: string, project_slug: string,
 *   mtime_ms: number, user_preview: string, generating: boolean, held: boolean }>>}
 */
async function discoverCursorRunsForPicker(homeDir = os.homedir(), options = {}) {
  const projectsRoot = path.join(homeDir, '.cursor', 'projects');
  const maxRuns =
  ...
  const staleMs = Number.isFinite(options.activeStaleMs) ? options.activeStaleMs : ACTIVE_GENERATION_STALE_MS;
  ...
  // Pass 1 — stat-only prefilter (readdir + stat, no tailing). Fold the newest sub-agent child
  // mtime into the recency test so a child-only-active parent is NOT dropped before we look at it.
  const candidates = [];
  for (const proj of projectEntries) {
    if (!proj.isDirectory()) continue;
    const transcriptsDir = path.join(projectsRoot, proj.name, 'agent-transcripts');
    ...
    try {
      ...
      continue; // numeric IDE slugs often have an empty/absent agent-transcripts dir
    }
    for (const runDir of runDirs) {
      if (!runDir.isDirectory()) continue;
      const runId = runDir.name;
      const runPath = path.join(transcriptsDir, runId);
      const jsonlPath = path.join(runPath, `${runId}.jsonl`);
      let st;
      try {
        st = fs.statSync(jsonlPath);
      } catch {
        continue; // no parent transcript file
      }
      const parentMtimeMs = st.mtimeMs || 0;
      const child = newestCursorSubagentChild(runPath);
      const lastActivityMs = Math.max(parentMtimeMs, child.mtimeMs);
      if (recentOnlyMs && lastActivityMs < nowMs - recentOnlyMs) continue;
      candidates.push({ runId, projectSlug: proj.name, jsonlPath, child, lastActivityMs });
    }
  }
  candidates.sort((a, b) => b.lastActivityMs - a.lastActivityMs);
  const top = candidates.slice(0, maxRuns);

  // Pass 2 — tail the parent (and, only when needed, the newest child) to classify `generating`.
  const runs = [];
  for (const cand of top) {
    let parentTail = '';
    try {
      const r = await readTranscriptTailText(cand.jsonlPath);
      parentTail = r.missing ? '' : r.tailText || '';
    } catch {
      parentTail = '';
    }
    const parentTurnEnded = cursorTranscriptTurnEnded(parentTail); // true | false | null
    const parentOpen = parentTurnEnded !== true; // null (partial write) fails open to "open"
    // The parent's own turn is done — but a still-running sub-agent keeps it in-flight. Tail the
    // newest child ONLY here (cheap: the stat already told us whether a fresh child exists).
    let childOpen = false;
    if (parentTurnEnded === true && cand.child.mtimeMs && cand.child.mtimeMs >= nowMs - staleMs) {
      let childTail = '';
      try {
        const r = await readTranscriptTailText(cand.child.path);
        childTail = r.missing ? '' : r.tailText || '';
      } catch {
        childTail = '';
      }
      childOpen = cursorTranscriptTurnEnded(childTail) !== true; // non-terminal child still running
    }
    const activeGen = applyActiveGenerationStaleCutoff(
      { generating: parentOpen || childOpen, last_activity_at: toIso(cand.lastActivityMs) },
      { mtimeMs: cand.lastActivityMs, nowMs, activeStaleMs: options.activeStaleMs }
    );
    runs.push({
      run_id: cand.runId,
      transcript_path: cand.jsonlPath,
      project_slug: cand.projectSlug,
      mtime_ms: cand.lastActivityMs,
      user_preview: parentTail
        ? latestUserWordsPreviewFromTailText(parentTail, PREVIEW_MAX_WORDS)
        : '',
      generating: activeGen.generating,
      // Held == kept in-flight solely by a running child (parent turn ended). Mirrors the live
      // store's `held`; when the parent turn is itself open the row stands on its own generating.
      held: activeGen.generating && !parentOpen && childOpen,
    });
  }
...
/**
 * Whether a Cursor agent transcript contains an AskQuestion tool_use on an assistant turn at or
 * after `sinceIso`. cursor-cli writes this row only after the user answers (not while the gate is
 * held), so it is the positive "answer recorded, agent continuing" signal for resuming a watch
 * paused on a chat_db question gate.
 * @param {string} raw full transcript JSONL text
 * @param {string} sinceIso ISO timestamp (typically the pause's clear_signal_at)
 * @returns {boolean}
 */
function cursorTranscriptHasAskQuestionSince(raw, sinceIso) {
  const sinceMs = Date.parse(sinceIso || '') || 0;
  for (const line of String(raw || '').split('\n')) {
    const record = parseCursorTranscriptLine(line);
    if (!record || record.role !== 'assistant') continue;
    if (!cursorRecordAfterLinkedAt(record, sinceMs)) continue;
    for (const block of cursorRecordContentBlocks(record)) {
      if (block && block.type === 'tool_use' && isCursorAskQuestionToolName(block.name)) return true;
    }
  }
  return false;
}
...
/**
 * Read the tracked transcript and report whether an AskQuestion tool_use was recorded after
 * `options.sinceIso` (the pause time for a question-gate resume).
 */
async function cursorTranscriptAskQuestionRecordedSince(cursorTracking, options = {}) {
  const sinceIso = typeof options.sinceIso === 'string' ? options.sinceIso : '';
  const raw = await readCursorTranscriptText(cursorTracking, options);
  return raw ? cursorTranscriptHasAskQuestionSince(raw, sinceIso) : false;
}

/**
 * Whether the transcript's LAST record is `turn_ended` — cursor-agent's true end-of-turn marker.
 *
 * cursor-agent fires a `stop` hook once per GENERATION, but a turn can span several generations:
 * a Task sub-agent finishing or a background wakeup injects a `<user_query>` record that starts a
 * new generation (with its own stop) in the same conversation. The single `turn_ended` transcript
 * record is appended only when the WHOLE turn is over (verified across every recorded cursor-cli
 * scenario, including multi-stop sub-agent/background runs), so "last record is turn_ended" is the
 * final-stop discriminator a bare stop hook lacks. Records carry no timestamps, so position (last
 * line) — not time — is the oracle; a new prompt appends a user record and flips this back to false.
 *
 * Returns true/false, or null when the transcript is empty/unavailable or its tail is unparseable
 * (unknown → callers fail open to the plain stop-hook behavior).
 * @param {string} raw full transcript JSONL text
 * @returns {boolean|null}
 */
function cursorTranscriptTurnEnded(raw) {
  const lines = String(raw || '').split('\n');
  for (let i = lines.length - 1; i >= 0; i -= 1) {
    const line = lines[i].trim();
    if (!line) continue;
    const record = parseCursorTranscriptLine(line);
    if (!record) return null; // unparseable tail (partial write) → unknown
    // Any parseable record that is not turn_ended (role records carry no `type`) = turn open.
    return record.type === 'turn_ended';
  }
  return null;
}

/**
 * Read the tracked conversation's transcript (local or ssh) and report whether the turn has
 * ended (last record is `turn_ended`). Returns null when the transcript is missing/unreadable —
 * callers treat unknown as "do not hold".
 */
async function cursorConversationTurnEnded(cursorTracking, options = {}) {
  let raw = '';
  try {
    raw = await readCursorTranscriptText(cursorTracking, options);
  } catch {
    return null;
  }
  if (!raw) return null;
  return cursorTranscriptTurnEnded(raw);
}
...
module.exports = {
  ...
  cursorTranscriptHasAskQuestionSince,
  cursorTranscriptAskQuestionRecordedSince,
  cursorTranscriptTurnEnded,
  cursorConversationTurnEnded,
