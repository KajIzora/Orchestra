...
// `invoke_subagent` is how agy DELEGATES work to a sub-agent. Unlike a normal tool, the parent
// does not block-and-return for it — the spawn's PreToolUse often has NO matching PostToolUse
// (the sub-agent's own Stop is the completion). So treating it as an "in-flight tool the parent
// is waiting on" is wrong: the marker would never clear and would jam the idle-completion backup
// for the whole 30-min cap. Cascade-wide quiescence already tracks the spawned sub-agents
// independently, so the parent's delegation call must NOT set an in-flight marker.
function isSubagentInvocationToolCall(toolCall) {
  return String(toolCall?.name || '').trim() === 'invoke_subagent';
}
