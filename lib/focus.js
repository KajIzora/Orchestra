...
function spawnLaunchCommand(command, spec) {
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      child.kill('SIGTERM');
      finish({ ok: false, command, error: `Command timed out after ${TIMEOUT_MS}ms`, output });
    }, TIMEOUT_MS);
...
function execLaunchCommand(command) {
  const directSpec = getDirectLaunchSpec(command);
  if (directSpec) return spawnLaunchCommand(command, directSpec);
      ...
      (err, stdout, stderr) => {
        if (err) {
          resolve({
            ok: false,
            command,
            error: err.killed ? `Command timed out after ${TIMEOUT_MS}ms` : (stderr || err.message),
            output: stdout || '',
          });
          return;
        }
...
async function runFocusCommands(commandOrCommands, opts = {}) {
  ...
  if (commands.length > 1 && isDesktopSwitchCommand(commands[0])) {
    ...
    const first = await execLaunchCommand(commands[0]);
    if (settleMs > 0) await delay(settleMs);
    const rest = await Promise.all(commands.slice(1).map((command) => execLaunchCommand(command)));
